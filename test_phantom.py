...
class DataliciousRequest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.server.start()
        cls.proxy = cls.server.create_proxy({"port":9912})
        cls.proxy_address = "--proxy=localhost:%s" % cls.proxy.port
        cls.service_args = [ cls.proxy_address, '--ignore-ssl-errors=yes', ] #So that HTTPS connections are possible

        # Creating the test for PhantomJS

        cls.driver = webdriver.PhantomJS(service_args=cls.service_args)
        cls.driver.set_window_size(1400, 1050)
        ...
        organic_link = cls.driver.find_element_by_class_name("r")
        click_link = organic_link.find_element_by_tag_name("a")
        click_link.click()
